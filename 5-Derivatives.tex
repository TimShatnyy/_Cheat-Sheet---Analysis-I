...
\tsIdea{Continuity / Differentiability at $x_0$}{
    \[
    \boxed{
    ...
    \]
    \(\tsPoint\) \textbf{Differentiability:}
    \[
    \boxed{
    f \text{ differentiable at an interior point }x_0
    \iff
    ...
    \begin{cases}
    f'_-(x_0)=f'_+(x_0)\in\mathbb R
    &\Rightarrow f'(x_0)\text{ exists},\\[1mm]
    f'_-(x_0)\neq f'_+(x_0)
    &\Rightarrow f'(x_0)\text{ does not exist},\\[1mm]
    \text{only one one-sided derivative exists}
    &\Rightarrow f'(x_0)\text{ does not exist}.
    \end{cases}
    \]
    \[
    \text{Example: }f(x)=|x|,
    \qquad
    f'_-(0)=-1\neq1=f'_+(0)
    \Rightarrow f'(0)\text{ does not exist}.
    \]
    \[
    \boxed{
    \begin{aligned}
    \operatorname{dom}(f)=[a,b]:\qquad
    f'_+(a)
    &=
    \lim_{h\to0^+}
    \frac{f(a+h)-f(a)}{h},\\
    ...
    \)
    \\
    \textbf{Continuity at $0$:}
    \[
    |f(x)|
    =
    \left|x^2\sin\!\left(\frac1x\right)\right|
    \le x^2
    \xrightarrow[x\to0]{}0=f(0).
    \]
    ...
    since
    \(
    \left|h\sin\!\left(\frac1h\right)\right|
    \le |h|
    \xrightarrow[h\to0]{}0.
    \)
    Thus \(f\) is continuous and differentiable at \(0\), with
    \[
    \boxed{f'(0)=0}.
    \]
}
...
\tsCor{Convexity via Second Derivative}{
    ...
    \begin{itemize}[noitemsep]
        \item \(f''(x) \ge 0 \iff f \text{ is convex (curved upward)}.\)
        \item \(f''(x) \leq 0 \Longleftrightarrow f \text{ is concave (curved downward)}\)
    \end{itemize}
}
\tsThe{Second Derivative Test}{
    Let $f$ be twice differentiable near $x_0$. Then:
    \begin{itemize}[noitemsep]
        ...
        \item If $f''(x_0)=0$ and $f''$ changes sign at $x_0$, then
        \(
        x_0 \text{ is a Wendepunkt}.
        \)

        \item If, in addition, $f'(x_0)=0$, then
        \(
        x_0 \text{ is a Sattelpunkt}.
        \)
    \end{itemize}
}
