...
\tsDef{Injective, Surjective, Bijective}{
    Let $f : X \to Y$.
    \begin{itemize}[noitemsep]
        \item $f$ is \emph{injective} if (different outputs for different inputs) \\
              \[
                  f(x_1) = f(x_2) \Rightarrow x_1 = x_2.
              \]
        \item $f$ is \emph{surjective} if  (every value in the codomain gets hit) \\
              \[
...
\tsThe{Sequential Characterization of Continuity}{
    Let \(f:D\subseteq\mathbb{R}\to\mathbb{R}\) and \(x_0\in D\).
    Then \(f\) is continuous at \(x_0\) if and only if, for every sequence
    \((x_n)_{n\in\mathbb N}\subseteq D\),
    \[
